...
where
    extract(year from g.time_label) between 2003 and 2015
...
(
select
min(results.game_date-p.game_date)
from wr.results p
where results.team_id=p.team_id
and results.game_date>p.game_date
group by results.game_date,results.team_id
);
...
min(results.game_date-p.game_date)
from wr.results p
where results.opponent_id=p.team_id
and results.game_date>p.game_date
group by results.game_date,results.opponent_id
);
