...
join major_league.teams t1
  on (t1.team_id) = (g.team_id)
join major_league.teams t2
  on (t2.team_id) = (g.opponent_id)

join major_league._zinb_schedule_factors sf1
  on (sf1.team_id)=(t1.team_id)
join major_league._zinb_schedule_factors sf2
...
join major_league._zinb_basic_factors i
  on (i.factor)=('(Intercept)')

join major_league._zinb_basic_factors z
  on (z.factor)=('pz')
