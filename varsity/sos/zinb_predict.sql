...
from varsity.games g

join varsity.teams t1
  on (t1.team_id) = (g.team_id)
join varsity.teams t2
  on (t2.team_id) = (g.opponent_id)

join varsity._zinb_schedule_factors sf1
  on (sf1.team_id)=(t1.team_id)
join varsity._zinb_schedule_factors sf2
  on (sf2.team_id)=(t2.team_id)

join varsity._factors o
  on (o.parameter,o.level)=('field','offense_home')
join varsity._factors d
  on (d.parameter,d.level)=('field','defense_home')

join varsity._zinb_basic_factors i
  on (i.factor)=('(Intercept)')

join varsity._zinb_basic_factors z
  on (z.factor)=('pz')
